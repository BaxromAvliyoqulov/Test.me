...
const friendsDir = path.join(__dirname, '..', 'src', 'components', 'friends');
if (!fs.existsSync(friendsDir)) {
  fs.mkdirSync(friendsDir, { recursive: true });
}

// 1. FriendsSidebar.vue
const friendsSidebar = `<template>
...
// 2. FriendsSearchGrid.vue
const friendsSearchGrid = `<template>
...
fs.writeFileSync(path.join(friendsDir, 'FriendsSidebar.vue'), friendsSidebar);
fs.writeFileSync(path.join(friendsDir, 'FriendsSearchGrid.vue'), friendsSearchGrid);

console.log('Successfully created friends components');
